...
def _load_dotenv():
    """Read KEY=value lines from .env next to this file into os.environ (existing env wins)."""
...
_load_dotenv()
MODEL = os.environ.get("TURING_MODEL", "claude-haiku-4-5")   # fastest current model
GAME_SECONDS = int(os.environ.get("TURING_SECONDS", "90"))
NEVER_NAME = {x.strip().lower() for x in os.environ.get("TURING_NEVER_NAME", "john").split(",") if x.strip()}   # judge names the bot must never say
MOCK = os.environ.get("MOCK_AI") == "1"                       # plumbing test without API calls

_client: Optional["anthropic.AsyncAnthropic"] = None


def get_client() -> "anthropic.AsyncAnthropic":
    """Built on first use so the server boots even before ANTHROPIC_API_KEY is set."""
    global _client
    if _client is None:
        ws = os.environ.get("ANTHROPIC_WORKSPACE_ID")
        # keys not scoped to a workspace must name one on every request
        _client = anthropic.AsyncAnthropic(default_headers={"anthropic-workspace-id": ws} if ws else None,
                                           timeout=6.0, max_retries=1)
    return _client

if not MOCK and not (os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_AUTH_TOKEN")):
    print("!! ANTHROPIC_API_KEY is not set. The AI player will stay silent.")
    print("!! Run:  ANTHROPIC_API_KEY=sk-ant-... python3 server.py     (or MOCK_AI=1 to test plumbing)")
...
class Game:
    def __init__(self, gid: str):
        ...
        self.sockets: Dict[str, Optional[WebSocket]] = {"judge": None, "player": None}
        labels = ["A", "B"]
    ...
    def snapshot(self, role: str) -> dict:
        typing = {"A": False, "B": False}      # typing indicators are off: they were a timing tell either way
        msgs = list(self.messages)
        if role == "player" and self.held_first:
            msgs.append(dict(self.held_first, queued=True))
        snap = {
            "type": "state",
            "game": self.id,
            "phase": self.phase,
            ...
            "judgeName": self.names["judge"],
            "mock": MOCK,
            "nokey": (not MOCK) and not (os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_AUTH_TOKEN")),
            "model": MODEL,
        }
        snap["nextGame"] = self.next_game
        if self.phase == "reveal":
            snap["reveal"] = {
                "human": self.human_label,
                "ai": self.ai_label,
                "pick": self.vote["pick"],
    ...
    async def broadcast(self):
        for role, ws in self.sockets.items():
            if ws is None:
                continue
            try:
    ...
    async def on_join(self, role: str, ws: WebSocket, name: str = "", hints: Optional[dict] = None):
        ...
        asyncio.create_task(try_match())     # someone waiting in the queue may fit the empty seat here
        asyncio.create_task(notify_queue())
        name = re.sub(r"[^\w \-'.]", "", str(name))[:24].strip()
        if name:
            self.names[role] = name
        if role == "player" and isinstance(hints, dict):
            self.hints = {k: str(v)[:40] for k, v in hints.items() if k in ("mobile", "tz", "localTime", "day", "platform")}
        if self.phase == "lobby" and all(self.sockets.values()):
            self.phase = "opening"
            self.task = asyncio.create_task(self.ai_loop())
        await self.broadcast()

    async def on_leave(self, role: str, ws: WebSocket):
        if self.sockets.get(role) is ws:
            self.sockets[role] = None
        await self.broadcast()
    ...
    async def on_vote(self, pick: str, reason: str):
        if self.phase not in ("opening", "live", "voting") or pick not in ("A", "B"):
            return
        self.vote = {"pick": pick, "reason": reason.strip()[:1000]}
        self.phase = "reveal"
        if self.task:
            self.task.cancel()
        try:
            ...
            print("transcript save failed:", repr(e))
        await self.broadcast()

    # -- AI brain -----------------------------------------------------------
    # The AI keeps ONE plan at a time: a response to the newest non-AI message.
    # While the human types, it pre-drafts against their live draft so the reply
    # is ready the moment they hit send, then lands it relative to the human's
    # send time (paced to how fast the human answers) instead of on its own clock.
    ...
    # ... opening: human's first message is held; AI pre-drafts while they type
...
Keep it under 220 words. No preamble."""
    ...
    async def generate(self, trigger: str, force: bool = False) -> List[str]:
        ...
        if MOCK:
            ...
            pool = ["lol ok", "wait which one of u is the bot", "bruh", "thats such a bot answer", "idk man", "no ur the bot"]
            return [random.choice(pool)] if random.random() < 0.8 or trigger == "sent" else []
...
# ----------------------------------------------------------------------------
games: Dict[str, Game] = {}
queue: Dict[str, List[WebSocket]] = {"judge": [], "player": []}   # strangers waiting for a match, by role


def open_seat(role: str) -> Optional[str]:
    """A standby room where the other human is already waiting and this role's seat is empty."""
    other = "player" if role == "judge" else "judge"
    for gid, g in games.items():
        if g.phase == "lobby" and g.sockets[role] is None and g.sockets[other] is not None:
            return gid
    return None


def waiting_counts() -> dict:
    counts = {"judge": len(queue["judge"]), "player": len(queue["player"])}
    for g in games.values():
        if g.phase == "lobby":
            for r in ("judge", "player"):
                if g.sockets[r] is not None and g.sockets["player" if r == "judge" else "judge"] is None:
                    counts[r] += 1
    return counts


async def notify_queue():
    payload = json.dumps({"type": "queued", "waiting": waiting_counts()})
...
async def try_match():
    # first fill open seats in rooms where someone is already waiting alone
    for role in ("judge", "player"):
        while queue[role]:
            gid = open_seat(role)
            if not gid:
                break
            ws_ = queue[role].pop(0)
            try:
                await ws_.send_text(json.dumps({"type": "matched", "game": gid, "role": role}))
            except Exception:
                pass
    while queue["judge"] and queue["player"]:
        j, pl = queue["judge"].pop(0), queue["player"].pop(0)
        gid = secrets.token_hex(3)
        get_game(gid)
        for ws_, role_ in ((j, "judge"), (pl, "player")):
            try:
                await ws_.send_text(json.dumps({"type": "matched", "game": gid, "role": role_}))
            except Exception:
                pass
    ...
    try:
        while True:
            ...
            if kind == "join":
                ...
                r = data.get("role")
                if not gid or r not in ("judge", "player"):
                    continue
                g = get_game(gid)
                old = g.sockets[r]
                if old is not None and old is not ws:
                    try:
                        await old.close()   # reload/reconnect wins the seat
                    except Exception:
                        pass
                game, role = g, r
                await g.on_join(r, ws, data.get("name", ""), data.get("hints"))
            elif kind == "queue":
                r = data.get("role")
                if r in ("judge", "player") and ws not in queue[r]:
                    for v in queue.values():            # switching roles: leave the other queue
                        if ws in v:
                            v.remove(ws)
                    queue[r].append(ws)
                    await try_match()
                ...
                await game.on_message(role, str(data.get("text", "")))
            elif kind == "vote" and role == "judge":
                await game.on_vote(str(data.get("pick", "")), str(data.get("reason", "")))
            elif kind == "rematch" and game.phase == "reveal":
                if not game.next_game:
                    game.next_game = secrets.token_hex(3)
                    get_game(game.next_game)               # both clients move over with their roles
